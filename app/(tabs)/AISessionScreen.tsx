...
const EXERCISE_ROUTINES: Record<ExerciseType, StretchStep[]> = {
  羽球: [{ title: '股四頭肌伸展', hint: '單腳站立，手拉同側腳踝', duration: 25 }, { title: '膕繩肌伸展', hint: '臀部向後推，放鬆大腿後側', duration: 25 }, { title: '小腿腓腸肌伸展', hint: '雙手扶牆，一腳向後踩平', duration: 25 }, { title: '肩膀與背部放鬆', hint: '一隻手橫過胸前，另一手扣住', duration: 20 }],
  籃球: [{ title: '臀部肌肉放鬆', hint: '坐姿，一腳跨過另一腳膝蓋轉向', duration: 30 }, { title: '股四頭肌伸展', hint: '單腳站立，手拉同側腳踝', duration: 30 }, { title: '核心與背部延展', hint: '雙手向上伸直，向左右側彎', duration: 25 }],
  跑步: [{ title: '小腿深層伸展', hint: '腳跟下沉延展小腿腓腸肌', duration: 30 }, { title: '大腿後側膕繩肌', hint: '一腳前伸腳尖勾起，身體前傾', duration: 30 }, { title: '臀大肌伸展', hint: '仰臥抱膝至胸前維持', duration: 30 }],
  游泳: [{ title: '肩旋轉肌群', hint: '牆角前傾或雙手後扣伸展肩膀', duration: 25 }, { title: '背闊肌伸展', hint: '單手向上抓柱子，身體向側推', duration: 25 }, { title: '肱三頭肌伸展', hint: '一手扶肘部，手掌摸向背後', duration: 20 }],
  桌球: [{ title: '腕部與手指放鬆', hint: '掌心向前，另一手將指尖向後拉', duration: 20 }, { title: '肩膀旋轉放鬆', hint: '緩慢旋轉肩關節，放鬆揮拍手', duration: 20 }, { title: '下肢穩定肌伸展', hint: '側壓腿，伸展大腿內側', duration: 20 }],
  排球: [{ title: '胸大肌擴張', hint: '雙手後扣挺胸，緩慢抬高', duration: 25 }, { title: '擊球手臂伸展', hint: '手臂伸直壓牆旋轉身體', duration: 25 }, { title: '小腿爆發肌伸展', hint: '弓箭步維持，腳跟踩實', duration: 25 }],
};
...
export default function AISessionScreen() {
      ...
      if (photo?.base64) {
        ...
        if (poses && poses.length > 0) {
          ...
          if (score > 0.2) { 
            ...
            Speech.speak('請確保全身入鏡', { language: 'zh-TW' });
          }
        }
        imageTensor.dispose();
      }
  ...
  const nextStep = () => {
    if (currentStepIndex < routine.length - 1) {
      ...
      Speech.speak('訓練結束', { language: 'zh-TW' });
      stopSession();
    }
  };
...
const styles = StyleSheet.create({
  ...
  sectionTitle: { fontSize: 18, fontWeight: 'bold', color: '#64748B', marginBottom: 15, fontFamily: 'Zen' },
  modeRow: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 30 },
  modeCard: { width: '30%', padding: 20, backgroundColor: '#FFF', borderRadius: 20, alignItems: 'center', borderWidth: 2, borderColor: '#E2E8F0' },
  ...
  modeLabel: { fontSize: 14, color: '#94A3B8', fontWeight: '600', fontFamily: 'Zen' },
  modeLabelActive: { color: '#3498DB' },
  paramCard: { backgroundColor: '#FFF', borderRadius: 30, padding: 25, elevation: 5, shadowColor: '#000', shadowOpacity: 0.05, shadowRadius: 10 },
  paramItem: { marginBottom: 25 },
  labelRow: { flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 12 },
  paramLabel: { fontSize: 15, color: '#64748B', fontWeight: '600', fontFamily: 'Zen' },
  chipGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 10 },
  chip: { paddingHorizontal: 18, paddingVertical: 10, borderRadius: 12, backgroundColor: '#F1F5F9' },
  chipActive: { backgroundColor: '#3498DB' },
  chipText: { color: '#94A3B8', fontWeight: '600', fontFamily: 'Zen' },
  ...
  startBtnText: { color: '#FFF', fontSize: 18, fontWeight: 'bold', fontFamily: 'Zen' },
  cameraOverlay: { flex: 1, justifyContent: 'space-between', padding: 30, backgroundColor: 'rgba(0,0,0,0.2)' },
  hudCard: { backgroundColor: '#FFF', padding: 20, borderRadius: 25, alignItems: 'center' },
  hudTitle: { fontSize: 22, fontWeight: 'bold', color: '#0F172A', fontFamily: 'Zen' },
  timerCircle: { width: 80, height: 80, borderRadius: 40, borderWidth: 5, borderColor: '#3498DB', justifyContent: 'center', alignItems: 'center', marginTop: 15 },
  hudTimer: { fontSize: 30, fontWeight: 'bold', color: '#3498DB' },
  detectBtn: { backgroundColor: '#3498DB', padding: 20, borderRadius: 50, alignItems: 'center' },
  detectBtnText: { color: '#FFF', fontWeight: 'bold', fontSize: 18, fontFamily: 'Zen' },
  exitBtn: { alignSelf: 'center', padding: 10 },
  exitBtnText: { color: '#FFF', fontWeight: 'bold', fontFamily: 'Zen' },
